...
class InfomationBodyDartCubit extends Cubit<InfomationBodyDartState> {
  InfomationBodyDartCubit({required InsretDataUseCase insretDataUseCase})
      : _insretDataUseCase = insretDataUseCase,
        super(const InfomationBodyDartState());
  late final InsretDataUseCase _insretDataUseCase;
  ...
  void checkValid(String value) {
    if (value.trim().isNotEmpty) {
      if (value.trim().contains('.') || value.trim().contains(' ')) {
        ...
        emit(state.copyWith(valid: 'Hãy nhập số lớn hơn 16 và nhỏ hơn 100'));
      } else {
