...
class CalendarItemWidget extends StatelessWidget {
  CalendarItemWidget(
      this.sizeScreen, this.dateTime, this.choose, this.index, this.calories,
      {super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () {
        if (checkDay) {
          context.read<NutritionScreenCubitCubit>().getTrueValue(index);
        }
      },
      child: Container(
        ...
        decoration: BoxDecoration(
            color: (dateTimeNow.difference(dateTime).inDays >= 0)
                ? choose
                    ? calories
                        ? AppColor.pink
                        : AppColor.blue
